        ...
        if self.supabase_service_key:
            try:
                service_client = create_client(
                    ...
                    options=ClientOptions(timeout=self.connection_timeout)
                )
...
class UserDB:
    def __init__(self, supabase_client):
        ...
        self.db = supabase_client._client  # 直接访问底层客户端以支持高级操作
...
class StorageDB:
    ...
    def delete_file(self, bucket, file_path):
        ...
        try:
            ...
            # 记录删除日志（Pro版增强）
            try:
                self.client.table('file_uploads').delete()
                .eq('bucket', bucket)
    ...
    def get_file_metadata(self, bucket, file_path):
        ...
        try:
            # 查询数据库中的文件信息
            response = self.client.table('file_uploads')
            .select('*')
            .eq('bucket', bucket)
            .eq('file_path', file_path)
            .execute()
