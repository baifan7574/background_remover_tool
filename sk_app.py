...
def add_watermark():
    ...
    try:
        ...
        print(f"🖼️ 水印参数 - 文字: {watermark_text}, 位置: {watermark_position}, 透明度: {opacity}, 字体大小: {font_size}, 颜色: {font_color}")
        ...
        print(f"🎨 水印颜色: RGB({r}, {g}, {b}), 透明度: {alpha}/255")
        
        # 绘制水印（添加描边使其更清晰可见）
        # 先绘制描边（黑色半透明，提高可见度）
        stroke_width = max(2, font_size // 15)
        stroke_alpha = min(255, alpha + 50)  # 描边稍暗一些
        ...
        for offset_x, offset_y in offsets:
            draw.text((position[0] + offset_x, position[1] + offset_y), watermark_text, 
                     font=font, fill=(0, 0, 0, stroke_alpha))
        
        # 再绘制主文字
        draw.text(position, watermark_text, font=font, fill=(r, g, b, alpha))
        
        print(f"✅ 水印已绘制 - 位置: {position}, 文字: {watermark_text}")
        ...
        # 保存处理后的图片
        output_filename = f'watermarked_{uuid.uuid4().hex[:8]}.png'
        output_path = os.path.join(app.config['UPLOAD_FOLDER'], output_filename)
        output_image.save(output_path, 'PNG')
        ...
        record_tool_usage(
            user_id,
            'add_watermark',
            {'watermark_text': watermark_text, 'watermark_position': watermark_position, 'opacity': opacity},
            {'output_filename': output_filename}
        )
